function [u] = MSFOndas(R,xc,T,M,u0,v0,dist,Nb,lambda0,lambda1,h,g,u_exacta)
% -------------------------------------------------------------------------
% Consideramos un problema de la forma:
%
% u_tt - Lap(u) = h(x,t) en B(xc;R)x(0,T]
% u = g(x,t)             sobre fr(B(xc;R))x(0,T]
% u(x,0) = u0            en B(xc;R)
% u_t(x,0) = v0          en B(xc;R)
%
% Entradas:
%
% R = Radio de la bola.
...
% v0 = Valor de la derivada temporal en el instante inicial t = 0.
...
% instante de tiempo, calculado mediante MSF_MSPEliptico.
% -------------------------------------------------------------------------
...
% La solución en t_2 la aproximamos usando la derivada en el instante
% inicial según se ha visto en el desarrollo teórico.
...
for n = 2:M
    ...
    tn1 = t(n+1);
    h1 = @(x) h(x,tn1);
    h_elipt = @(x) h1(x) + k2*(2*un(x) - un_1(x));
    g1 = @(x) g(x,tn1);
...
% -------------------------------------------------------------------------
% Vamos a ir construyendo a la vez la representación de la solución y del 
% error, ya que el proceso es análogo. Es por esto, para no doblar los 
% bucles, haremos los dos desarrollos simultáneamente.
% -------------------------------------------------------------------------
...
saveas(figure(1),'SolucionMetPbOndasDir.png');
saveas(figure(2),'SolucionExactaPbOndasDir.png');
saveas(figure(3),'ErrorPbOndasDir.png');
